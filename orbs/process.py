...
class Spectrum(HDFCube):
    ...
    def _get_flux_calibration_function_path(self):
        """Return the path to the flux calibration function.
        The flux calibration function is used to calibrate the data
        from ADu to erg s-1 cm-2 A-1. It includes the calibration vector
        and the calibration coefficient obtained with the standard star
        datacube and images.
        """
        return self._data_path_hdr + "flux_calibration_function.fits"

    def _get_modulation_ratio_path(self):
        """Return the path to the modulation ratio."""
        return self._data_path_hdr[:-9] + "InterferogramMerger.modulation_ratio.fits"

    def _get_flux_calibration_function_header(self):
        """Return the header of the flux calibration function"""
        return (self._get_basic_header('flux_calibration_function')
                + self._project_header)
    ...
    def calibrate(self, filter_name, step, STEP_NB, order,
        ...
        :param flux_calibration_coeff: (Optional) The flux
          calibration vector is adjusted with the calibration
          coeff. The flux calibration coefficient should be close to 1.
          It is used to ajust the calibration vector for global sky absorption or
          variation of the transmission of the system between the observation of
          the stardard datacube and the science target.
        ...
        def _calibrate_spectrum_column(spectrum_col, filter_function,
            ...
            for icol in range(spectrum_col.shape[0]):
                ...
                # First convert to dS/dsigma. Only makes sense if wavenumber=True
                channel_cm1 = orb.cutils.get_cm1_axis_step(ispectrum.shape[0],step,corr=corr)
                ispectrum /= channel_cm1
                # Now convert to dS/dlambda. Only makes sense if wavenumber=True
                ispectrum *= axis_corr_cm1_lowres**2 / 1e8
                ...
                if flux_calibration_function is not None:
                    ...
                    flux_corr[0:int(filter_min_pix)] = flux_corr[(int(filter_min_pix) + 1)]
                    flux_corr[int(filter_max_pix):] = flux_corr[(int(filter_max_pix) - 1)]
                ...
                if spectral_calibration or not wavenumber:
                    ...
                    spectrum_final = spectrum_highres

                result_col[icol,:] = spectrum_final
        ...
        if flux_calibration_vector[0] is not None:
            ...
            mean_flux_calib_vector = orb.utils.photometry.compute_mean_star_flux(
                ...
                filter_vector)
            logging.info('Mean flux calib vector before adjustment with flux calibration coeff: {} '.format(
                mean_flux_calib_vector))

        self.write_fits(self._get_flux_calibration_function_path(),
                        flux_calibration_function(np.float64(flux_calibration_axis)),
                        fits_header=self._get_flux_calibration_function_header(),
                        overwrite=self.overwrite)
    ...
    def get_flux_calibration_coeff(self,
        ...
        .. warning:: This calibration coeff takes Modulation
            Efficiency into account. A more representative calibration
            coefficient would be using a mesured modulation
            efficiency that could be different for the
	        flux_calibration_vector and the science cube.
        """
        ...
        std = Standard(std_name, instrument=self.instrument,
                       ncpus=self.ncpus)
        # Here th_spectrum_axis is regular in cm1 (wavenumber=True)
        th_spectrum_axis, th_spectrum = std.get_spectrum(
            step, order, STEP_NB,
            wavenumber=True, corr=corr)
        ...
        th_spectrum *= filter_vector
        # flux_calibration_vector includes ME. Hence for unmodulated light, we need to
        # divide by ME to eliminate it.
        th_spectrum /= ME

        # Calculate the correction for the atmospheric extinction.
        # Beware this is computed on a regular nm axis, hence the interpolation
        atm_trans = orb.utils.photometry.get_atmospheric_transmission(self._get_atmospheric_extinction_file_path()
                                                                      ...
                                                                      corr=std_corr)
        cm_axis_irreg = orb.utils.spectrum.nm2cm1(orb.utils.spectrum.create_nm_axis(std_STEP_NB,step,order,corr=std_corr))
        atm_trans_function = interpolate.UnivariateSpline(cm_axis_irreg[::-1].astype(np.float64),atm_trans[::-1],s=0,k=3)
        atm_trans = atm_trans_function(th_spectrum_axis.astype(np.float64))
        ...
        # First convert th_spectrum from ergs/s/cm2/A into ergs/s/cm2/cm1
        th_spectrum *= 10 * 1e7/th_spectrum_axis**2
        th_spectrum = np.diff(th_spectrum_axis) * th_spectrum[:-1]
        ...
        logging.info('Aperture flux of the standard star in camera 2 is {} [+/-{}] ADU/s'.format(std_flux2/modulation_ratio, std_flux_err2))

        coeff = std_th_flux / (std_flux1 + std_flux2/modulation_ratio) # erg/cm2/ADU
        
        logging.info('Flux calibration coeff: {} '.format(coeff))

        return coeff
        

    def get_flux_calibration_vector(self, std_spectrum_path, std_name,
                                    filter_name, std_airmass=1.):
        """
        ...
        Note that it contains the modulation efficiency.
        ...
        .. warning:: This flux calibration vector is computed from a
            spectrum which as a given modulation efficiency. It is not
            normalized to a modulation efficiency of 100% but the
            calibration coefficient computed from standard images includes it.
    """
        ...
        # channel size in cm1, to convert spectrum to density in ADU/s/cm1
        channel_cm1 = orb.cutils.get_cm1_axis_step(std_step_nb,std_step,corr=std_corr)
        re_spectrum /= channel_cm1
        # Now convert in density per angstroem
        convert = std_cm1_axis**2 / 1e8 # nu**2/1e7 / 10
        re_spectrum *= convert
        ...
        # Beware that it is computed on a regularly sampled nm axis, hence the interpolation
        atm_trans = orb.utils.photometry.get_atmospheric_transmission(
            ...
            corr=std_corr)
        cm_axis_irreg = orb.utils.spectrum.nm2cm1(orb.utils.spectrum.create_nm_axis(std_step_nb,std_step,std_order,corr=std_corr))
        atm_trans_function=interpolate.UnivariateSpline(cm_axis_irreg[::-1].astype(np.float64),atm_trans[::-1],s=0,k=3)
        atm_trans = atm_trans_function(th_spectrum_axis.astype(np.float64))
